...
- **Capacity model**: distribution template stays small; **default first-provision 32 GiB logical sparse** (sparse, `32*1024^3=34359738368`, physical stays ~1G until used) because workloads exhausted 2 GiB (32G default prevents early no-space) (`no space left on device`). Documented sparse: logical != physical. Supported `G/GiB/M/MiB` (e.g. `12G 16G 24G 32G`), integer only, rejects zero/negative/malformed/overflow/` <32G` (minimum 8G enforced)/shrink with clear messages.
