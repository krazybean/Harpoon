...
xcrun swiftc -target "arm64-apple-macosx$HARPOON_MIN_MACOS" \
  "$SCRIPT_DIR/Sources/RuntimeConfig.swift" "$SCRIPT_DIR/Sources/HostPathTranslator.swift" "$SCRIPT_DIR/Sources/Lifecycle.swift" "$SCRIPT_DIR/Sources/VMManager.swift" "$SCRIPT_DIR/Sources/Bridges.swift" "$SCRIPT_DIR/Sources/PortForwardManager.swift" "$SCRIPT_DIR/Sources/HarpoonDockerCLI.swift" "$SCRIPT_DIR/Sources/HarpoonDiskCLI.swift" "$SCRIPT_DIR/Sources/HarpoonMgmtCLI.swift" "$SCRIPT_DIR/Sources/UnixBridge.swift" "$SCRIPT_DIR/Sources/BalloonBridge.swift" "$SCRIPT_DIR/Sources/MgmtBridge.swift" "$SCRIPT_DIR/Sources/PortForwardBridge.swift" "$SCRIPT_DIR/Sources/HarpoonCLI.swift" "$SCRIPT_DIR/Sources/ContainerCLI.swift" "$SCRIPT_DIR/Sources/main.swift" \
